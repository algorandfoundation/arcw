    ...
    'b: 'a,
{
    ...
    pub(crate) arcs: &'b HashMap<&'b Path, Result<InnerContext<'b>, &'b crate::Error>>,
    #[educe(Debug(ignore))]
    ...
    'b: 'a,
{
    ...
    pub fn arc(&self, path: &Path) -> Result<Context<'b, 'b>, &crate::Error> {
        let origin = self
            .origin()
        ...
        let inner = match self.arcs.get(key.as_path()) {
            ...
            None => panic!("no arc found for key `{}`", key.display()),
        };
...
pub struct FetchContext<'a> {
    ...
    pub(crate) body: &'a AstNode<'a>,
    pub(crate) arcs: RefCell<HashSet<PathBuf>>,
}
...
impl<'a> FetchContext<'a> {
    ...
    pub fn fetch(&self, path: PathBuf) {
        self.arcs.borrow_mut().insert(path);
    }
}
